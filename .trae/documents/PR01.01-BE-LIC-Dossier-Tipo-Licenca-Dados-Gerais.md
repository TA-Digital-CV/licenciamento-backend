...
CREATE INDEX idx_license_parameter_created_date ON t_license_parameter(created_date DESC);
...
* Unidades de validade dinâmicas (T\_OPTIONS)
...
CREATE OR REPLACE FUNCTION update_license_parameter_last_modified_date()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER tr_license_parameter_last_modified_date
    BEFORE UPDATE ON t_license_parameter
    FOR EACH ROW
    EXECUTE FUNCTION update_license_parameter_last_modified_date();
...
### 6.4 Dados Iniciais t\_options - V004\_\_Insert\_License\_Parameter\_Options.sql
