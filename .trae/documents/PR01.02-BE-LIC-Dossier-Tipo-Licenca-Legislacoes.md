...
CREATE INDEX idx_legislation_created_date ON t_legislation(created_date DESC);
...
**Descrição**: Obtém a lista de tipos de legislação disponíveis (da tabela t_options).
...
- **Tipo de Legislação**: Obrigatório, deve existir na tabela t_options com ccode='TPLEG'
...
```sql
-- Trigger para atualizar last_modified_date automaticamente
CREATE OR REPLACE FUNCTION update_legislation_last_modified_date()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER tr_legislation_last_modified_date
    BEFORE UPDATE ON t_legislation
    FOR EACH ROW
    EXECUTE FUNCTION update_legislation_last_modified_date();
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        ...
        VALUES ('t_legislation', 'DELETE', OLD.id::text, row_to_json(OLD), OLD.last_modified_by, NOW());
        RETURN OLD;
    ELSIF TG_OP = 'UPDATE' THEN
        INSERT INTO audit_log (table_name, operation, record_id, old_values, new_values, changed_by, changed_at)
        VALUES ('t_legislation', 'UPDATE', NEW.id::text, row_to_json(OLD), row_to_json(NEW), NEW.last_modified_by, NOW());
        RETURN NEW;
...
INSERT INTO t_options (ccode, ckey, cvalue, locale, sort_order, active, description, created_by, last_modified_by)
VALUES
